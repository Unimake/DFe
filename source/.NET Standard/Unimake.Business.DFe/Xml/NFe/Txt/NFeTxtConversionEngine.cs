...
namespace Unimake.Business.DFe.Xml.NFe.Txt
{
    internal sealed class NFeTxtConversionEngine
    {
        ...
        {
            var modalidadeBaseCalculo = this.LerInt32(XmlTag<DFeNFe.ICMS00>(nameof(DFeNFe.ICMS00.ModBC)), ObOp.Opcional, 1, 1);
            var icms = new DFeNFe.ICMS51
            {
                Orig = (OrigemMercadoria)this.LerInt32(XmlTag<DFeNFe.ICMS00>(nameof(DFeNFe.ICMS00.Orig)), ObOp.Obrigatorio, 1, 1),
                CST = this.LerString(XmlTag<DFeNFe.ICMS00>(nameof(DFeNFe.ICMS00.CST)), ObOp.Obrigatorio, 2, 2),
                ModBC = (ModalidadeBaseCalculoICMS)(modalidadeBaseCalculo > 0 ? modalidadeBaseCalculo : -1),
        ...
        private void ProcessarIcms90(int nProd, int lenPipesRegistro)
        {
            ...
            if ((icms.PMVAST ?? 0) + (icms.PRedBCST ?? 0) + icms.VBCST + icms.PICMSST + icms.VICMSST + icms.VBCFCPST + icms.PFCPST + icms.VFCPST <= 0)
            {
                icms.ModBCST = (ModalidadeBaseCalculoICMSST)(-1);
                icms.PMVAST = null;
                icms.PRedBCST = null;
            }
            if (icms.VICMSDeson <= 0)
            {
                icms.MotDesICMS = (MotivoDesoneracaoICMS)(-1);
            }
        ...
        private void ProcessarIcmsSn900(int nProd, int lenPipesRegistro)
        {
            ...
            var modalidadeBaseCalculoSt = this.LerInt32(XmlTag<DFeNFe.ICMS10>(nameof(DFeNFe.ICMS10.ModBCST)), ObOp.Opcional, 1, 1, true);
            icms.ModBCSTField = (ModalidadeBaseCalculoICMSST)(modalidadeBaseCalculoSt == 100 ? -1 : modalidadeBaseCalculoSt);
            ...
            var percentualCredito = this.LerDouble(this.TipoCampo42, XmlTag<DFeNFe.ICMSSN101>(nameof(DFeNFe.ICMSSN101.PCredSN)), ObOp.Opcional, this.CasasDecimais75, true);
            icms.PCredSN = percentualCredito < 0 ? (double?)null : percentualCredito;
            var valorCredito = this.LerDouble(TpcnTipoCampo.tcDouble2, XmlTag<DFeNFe.ICMSSN101>(nameof(DFeNFe.ICMSSN101.VCredICMSSN)), ObOp.Opcional, 15, true);
            icms.VCredICMSSN = valorCredito < 0 ? (double?)null : valorCredito;
            if (versaoNFe >= 4)
            {
                icms.VBCFCPST = this.LerDouble(TpcnTipoCampo.tcDouble2, XmlTag<DFeNFe.ICMS10>(nameof(DFeNFe.ICMS10.VBCFCPST)), ObOp.Obrigatorio, 15);
                icms.PFCPST = this.LerDouble(TpcnTipoCampo.tcDouble4, XmlTag<DFeNFe.ICMS10>(nameof(DFeNFe.ICMS10.PFCPST)), ObOp.Obrigatorio, 15);
        ...
        private void ProcessarPisAliquota(int nProd, int lenPipesRegistro)
        {
            ...
            var aliquota = this.LerDouble(TpcnTipoCampo.tcDouble4, XmlTag<DFeNFe.PISAliq>(nameof(DFeNFe.PISAliq.PPIS)), ObOp.Obrigatorio, 7);
            var valor = this.LerDouble(TpcnTipoCampo.tcDouble2, XmlTag<DFeNFe.PISAliq>(nameof(DFeNFe.PISAliq.VPIS)), ObOp.Obrigatorio, 15);
            if (cst != "01" && cst != "02")
            {
                detalhesOficiais[nProd].Imposto.PIS = new DFeNFe.PIS
                {
                    PISOutr = new DFeNFe.PISOutr { CST = cst, VBC = vbc, PPIS = aliquota, VPIS = valor }
                };
                return;
            }
            detalhesOficiais[nProd].Imposto.PIS = new DFeNFe.PIS
        ...
        private void ProcessarCofinsAliquota(int nProd, int lenPipesRegistro)
        {
            var cst = this.LerString(XmlTag<DFeNFe.ICMS00>(nameof(DFeNFe.ICMS00.CST)), ObOp.Obrigatorio, 2, 2);
            var vbc = this.LerDouble(TpcnTipoCampo.tcDouble2, XmlTag<DFeNFe.ICMS00>(nameof(DFeNFe.ICMS00.VBC)), ObOp.Obrigatorio, 15);
            var aliquota = this.LerDouble(this.TipoCampo42, XmlTag<DFeNFe.COFINSAliq>(nameof(DFeNFe.COFINSAliq.PCOFINS)), ObOp.Obrigatorio, this.CasasDecimais75);
            ...
            if (cst != "01" && cst != "02")
            {
                ...
                {
                    COFINSOutr = new DFeNFe.COFINSOutr { CST = cst, VBC = vbc, PCOFINS = aliquota, VCOFINS = valor }
                };
                return;
            }
            detalhesOficiais[nProd].Imposto.COFINS = new DFeNFe.COFINS
