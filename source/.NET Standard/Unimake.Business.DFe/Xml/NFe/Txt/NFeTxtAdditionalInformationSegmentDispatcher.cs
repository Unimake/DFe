using System;
using System.Collections.Generic;

namespace Unimake.Business.DFe.Xml.NFe.Txt
{
    /// <summary>
    /// Despacha os segmentos de informações adicionais e grupos complementares da nota.
    /// </summary>
    internal sealed class NFeTxtAdditionalInformationSegmentDispatcher
    {
        private readonly Dictionary<string, Action<int, int>> acoes;

        internal NFeTxtAdditionalInformationSegmentDispatcher(Dictionary<string, Action<int, int>> acoes)
        {
            this.acoes = acoes;
        }

        internal bool TentarDespachar(string codigoSegmento, int numeroProduto, int quantidadePipes)
        {
            Action<int, int> acao;
            if (!acoes.TryGetValue(codigoSegmento, out acao)) return false;

            acao(numeroProduto, quantidadePipes);
            return true;
        }
    }
}
