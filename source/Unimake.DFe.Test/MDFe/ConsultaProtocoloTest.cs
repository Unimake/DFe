        ...
        public void ConsultarProtocoloMDFe(UFBrasil ufBrasil, TipoAmbiente tipoAmbiente)
        {
            try
            {
                ...
                if(consultaProtocolo.Result.ProtMDFe != null)
                {
                    ...
                    {
                        Debug.Assert(consultaProtocolo.Result.ProtMDFe.InfProt.ChMDFe.Equals(xml.ChMDFe), "Webservice retornou uma chave do MDFe diferente da enviada na consulta.");
                    }
                }
            }
            catch(Exception ex)
