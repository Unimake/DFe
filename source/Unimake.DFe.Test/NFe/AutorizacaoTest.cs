...
namespace Unimake.DFe.Test.NFe
{
    ...
    public class AutorizacaoTest
    {
        ...
        public void EnviarNFeAssincrono(UFBrasil ufBrasil, TipoAmbiente tipoAmbiente)
        {
            ...
            {
                case UFBrasil.PR:
                    xml.NFe[0].InfNFeField.InfRespTec.IdCSRT = idCSRT;
                    xml.NFe[0].InfNFeField.InfRespTec.HashCSRT = hashCSRT;
                    break;

                case UFBrasil.SP:
                    xml.NFe[0].InfNFeField.InfRespTec.IdCSRT = idCSRT;
                    xml.NFe[0].InfNFeField.InfRespTec.CSRT = CSRT;
                    break;

                case UFBrasil.RS:
                    xml.NFe[0].InfNFeField.InfRespTec.IdCSRT = idCSRT;
                    xml.NFe[0].InfNFeField.InfRespTec.HashCSRT = "G8063VRTNDMO886SFNK5LDUDEI24XJ22YIPO41180678393592000146558900000006041028190697";
                    break;
            }
            ...
            switch (ufBrasil)
            {
                ...
                case UFBrasil.SP:
                    elementInfRespTec = (XmlElement)autorizacao.ConteudoXMLAssinado.GetElementsByTagName("infRespTec")[0];
                    Assert.True(elementInfRespTec.GetElementsByTagName("idCSRT")[0].InnerText == idCSRT, "idCSRT informado está diferente do valor gerado no XML.");
                    Assert.True(elementInfRespTec.GetElementsByTagName("hashCSRT")[0].InnerText == Converter.CalculateSHA1Hash(CSRT + xml.NFe[0].InfNFeField.Chave), "hasCSRT informado está diferente do valor gerado no XML.");
                    break;
            ...
            {
                Assert.False(string.IsNullOrWhiteSpace(autorizacao.Result.InfRec.NRec), "Não retornou o número do recibo no envio da NFe");
            }
        }
        ...
        public void EnviarNFeSincrono(UFBrasil ufBrasil, TipoAmbiente tipoAmbiente)
        {
            ...
            if (autorizacao.Result.CStat.Equals(104))
            {
                ...
                Assert.True(autorizacao.Result.ProtNFe.InfProt.ChNFe.Equals(xml.NFe[0].InfNFeField.Chave), "Não teve retorno do processamento no envio síncrono");
            }
        }
        ...
        /// <summary>
        /// Enviar uma NFe com CNPJ Alfanumérico no emissor e no destinatário
        /// </summary>
        [Fact]
        [Trait("DFe", "NFe")]
        public void EnviarNFeCNPJAlfanumerico()
        {
            var tipoAmbiente = TipoAmbiente.Producao;
            var ufBrasil = UFBrasil.PR;
            var xml = MontaXMLEnviNFeCNPJAlfanumerico(ufBrasil, tipoAmbiente);
        ...
        /// </summary>
        /// <returns></returns>
        private EnviNFe MontaXMLEnviNFeCNPJAlfanumerico(UFBrasil ufBrasil, TipoAmbiente tipoAmbiente)
        {
            var xml = new EnviNFe
            {
                Versao = "4.00",
