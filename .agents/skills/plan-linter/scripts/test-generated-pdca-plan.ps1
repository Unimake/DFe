...
try {
    foreach ($path in @(
        "README.md", "AGENTS.md", "docsplan/nfabi/CODEX-START-HERE.md",
        ...
        "docsplan/nfabi/planning/PROJECT-BRIEF.md", "docsplan/nfabi/planning/QUESTION-LEDGER.md", "docsplan/nfabi/planning/RISK-REGISTER.md",
        "docsplan/nfabi/architecture/PROJECT-VISION.md", "docsplan/nfabi/architecture/DECISIONS-LOCKED.md", "docsplan/nfabi/architecture/DATA-ARCHITECTURE.md",
        "docsplan/nfabi/architecture/INTEGRATION-CATALOG.md", "docsplan/nfabi/architecture/QUALITY-ATTRIBUTES.md", "docsplan/nfabi/architecture/LIMITS-CATALOG.md",
        "docsplan/nfabi/architecture/SECURITY-CLAIMS.md", "docsplan/nfabi/architecture/ERROR-OBSERVABILITY-MODEL.md", "docsplan/nfabi/architecture/VERSIONING-BUILD.md",
        "docsplan/nfabi/design/UI-DESIGN-SYSTEM.md", "docsplan/nfabi/security/THREAT-MODEL.md",
        "docsplan/nfabi/plans/INDEX.md", "docsplan/nfabi/plans/STATUS.md", "docsplan/nfabi/plans/PDCA.md", "docsplan/nfabi/plans/READINESS.md",
        "docsplan/nfabi/plans/DECISION-REGISTER.md", "docsplan/nfabi/plans/MODEL-CATALOG.md", "docsplan/nfabi/plans/STAGE-ID-MAP.md",
        "docsplan/nfabi/plans/ENVIRONMENT-MATRIX.md", "docsplan/nfabi/plans/TRACEABILITY.md", "docsplan/nfabi/plans/VALIDATION-MANIFEST.md",
        "docsplan/nfabi/plans/VALIDATION-REGISTRY.md", "docsplan/nfabi/plans/EXECUTION-GUARDRAILS.md", "docsplan/nfabi/plans/EVIDENCE-TEMPLATE.md",
        "docsplan/nfabi/plans/evidence/README.md", "docsplan/nfabi/testing/TEST-LAB.md"
    )) { Require-File $path }

    if (-not (Test-Path -LiteralPath "docsplan/nfabi/plans" -PathType Container)) {
        throw (($errors | Sort-Object -Unique | ForEach-Object { "ERROR: $_" }) -join "`n")
    }

    $brief = if (Test-Path -LiteralPath "docsplan/nfabi/planning/PROJECT-BRIEF.md") { Get-Content -Raw -LiteralPath "docsplan/nfabi/planning/PROJECT-BRIEF.md" } else { "" }
    ...
    if (-not $prefixMatch.Success) {
        if (@(Get-ChildItem -LiteralPath "docsplan/nfabi/plans" -File -Filter "PARTE-*.md" -ErrorAction SilentlyContinue).Count -gt 0) {
    ...
    if (Test-Path -LiteralPath "README.md") {
        ...
        foreach ($requiredReference in @("docsplan/nfabi/plans/PDCA.md", "docsplan/nfabi/plans/DECISION-REGISTER.md", "docsplan/nfabi/planning/RISK-REGISTER.md", "docsplan/nfabi/plans/MODEL-CATALOG.md", "docsplan/nfabi/plans/evidence/")) {
    ...
    if (Test-Path -LiteralPath "docsplan/nfabi/planning/QUESTION-LEDGER.md") {
        $ledger = Get-Content -Raw -LiteralPath "docsplan/nfabi/planning/QUESTION-LEDGER.md"
    ...
    if (Test-Path -LiteralPath "docsplan/nfabi/plans/STATUS.md") {
        $status = Get-Content -Raw -LiteralPath "docsplan/nfabi/plans/STATUS.md"
    ...
    $plans = @(Get-ChildItem -LiteralPath "docsplan/nfabi/plans" -File | Where-Object { $_.BaseName -match "^${prefix}-[0-9]{3}-" })
    $stageById = @{}
    foreach ($plan in $plans) {
        ...
        if ($stageById.ContainsKey($id)) { $errors.Add("docsplan/nfabi/plans: ID duplicado: $id") }
    ...
    if(($stageNumbers-join'|')-ne($expectedNumbers-join'|')){$errors.Add("docsplan/nfabi/plans: etapas devem formar sequência contínua de 000 em diante")}
    ...
    if (Test-Path -LiteralPath "docsplan/nfabi/plans/PDCA.md") {
        $pdca=Get-Content -Raw -LiteralPath "docsplan/nfabi/plans/PDCA.md"
        ...
        foreach($line in Get-Content "docsplan/nfabi/plans/PDCA.md"){
    ...
    if(Test-Path "docsplan/nfabi/plans/STAGE-ID-MAP.md"){
        $map=Get-Content -Raw "docsplan/nfabi/plans/STAGE-ID-MAP.md"
    ...
    if (Test-Path -LiteralPath "docsplan/nfabi/plans/INDEX.md") {
        $ids=@([regex]::Matches((Get-Content -Raw "docsplan/nfabi/plans/INDEX.md"),"\((${prefix}-[0-9]{3})-[^)]+\.md\)")|ForEach-Object {$_.Groups[1].Value}); Add-Duplicates $ids "INDEX.md"; Assert-Sequence $stageIds $ids "INDEX.md"
    }
    if (Test-Path -LiteralPath "docsplan/nfabi/plans/VALIDATION-REGISTRY.md") {
        $validationText=Get-Content -Raw "docsplan/nfabi/plans/VALIDATION-REGISTRY.md"
    ...
    $knownEnvironments=@();if(Test-Path "docsplan/nfabi/plans/ENVIRONMENT-MATRIX.md"){$knownEnvironments=@([regex]::Matches((Get-Content -Raw "docsplan/nfabi/plans/ENVIRONMENT-MATRIX.md"),'(?m)^\| (ENV-[A-Z0-9-]+) \|')|ForEach-Object{$_.Groups[1].Value});Add-Duplicates $knownEnvironments "ENVIRONMENT-MATRIX.md"}
    if(Test-Path "docsplan/nfabi/plans/VALIDATION-REGISTRY.md"){
        foreach($line in Get-Content "docsplan/nfabi/plans/VALIDATION-REGISTRY.md"){
    ...
    $knownDecisions=@();foreach($decisionFile in @("docsplan/nfabi/plans/DECISION-REGISTER.md","docsplan/nfabi/architecture/DECISIONS-LOCKED.md")){if(Test-Path $decisionFile){$knownDecisions+=@([regex]::Matches((Get-Content -Raw $decisionFile),'(?m)^\| (DEC-[0-9]{3}) \|')|ForEach-Object{$_.Groups[1].Value})}};$knownDecisions=@($knownDecisions|Sort-Object -Unique)
    $referencedDecisions=@()
    foreach($decisionSource in @(Get-ChildItem -Recurse -File -Path "docsplan/nfabi" -Include "*.md","*.json" -ErrorAction SilentlyContinue)){
    ...
    $knownRisks=@();if(Test-Path "docsplan/nfabi/planning/RISK-REGISTER.md"){$knownRisks=@([regex]::Matches((Get-Content -Raw "docsplan/nfabi/planning/RISK-REGISTER.md"),'(?m)^\| (RISK-[0-9]{3}) \|')|ForEach-Object{$_.Groups[1].Value});Add-Duplicates $knownRisks "RISK-REGISTER.md"}
    $knownRequirements=@();$requirementsByStage=@{}
    if(Test-Path "docsplan/nfabi/plans/TRACEABILITY.md"){
        foreach($line in Get-Content "docsplan/nfabi/plans/TRACEABILITY.md"){
            if($line-notmatch'^\| (REQ-[0-9]{3}) \|'){continue};$cols=@($line.Trim('|').Split('|')|ForEach-Object{$_.Trim()});$req=$cols[0]
            $knownRequirements+=$req
            if($cols.Count-lt 7 -or $cols[3]-notin$stageIds -or [string]::IsNullOrWhiteSpace($cols[4]) -or $cols[5]-notmatch[regex]::Escape("docsplan/nfabi/plans/evidence/$($cols[3])")){$errors.Add("TRACEABILITY.md: etapa/validação/evidência inválida em $req");continue}
    ...
    $catalogProfiles=@{}
    if (Test-Path -LiteralPath "docsplan/nfabi/plans/MODEL-CATALOG.md") {
        foreach($line in Get-Content "docsplan/nfabi/plans/MODEL-CATALOG.md"){
    ...
    foreach($id in $stageIds){
        $mp="docsplan/nfabi/plans/manifests/$id.json"; Require-File $mp
        ...
        if($stageNumber-gt 0){
            ...
            foreach($dependency in $dependencies){
                ...
                if([int]$dependency.Substring($dependency.Length-3)-ge$stageNumber){
                    $dependencyManifestPath="docsplan/nfabi/plans/manifests/$dependency.json";$dependencyType=""
        ...
        if(-not(Test-Path $m.normative_plan)){$errors.Add("${mp}: normative_plan inexistente")}; if($m.dossier-ne"docsplan/nfabi/plans/evidence/$id"){$errors.Add("${mp}: dossier incorreto")}
        ...
        if($id-in@("${prefix}-000","${prefix}-001")){
            ...
            foreach($subtree in @($m.allowed_subtrees)){if($subtree-notmatch'^(docsplan/nfabi|\.agents)(/|$)'){$errors.Add("${mp}: planejamento permite produto: $subtree")}}
        }
        if($m.stage_type-eq"REPLANNING"){
            ...
            foreach($subtree in @($m.allowed_subtrees)){if($subtree-notmatch'^(docsplan/nfabi|\.agents)(/|$)'){$errors.Add("${mp}: REPLANNING permite produto: $subtree")}}
        }
        ...
        if($rowState-in@("IN_PROGRESS","BLOCKED","REWORK")){Require-File "docsplan/nfabi/plans/evidence/$id/EVIDENCE.md"}
        if($rowState-in@("DELIVERED_FOR_REVIEW","APPROVED")){foreach($f in @("DELIVERY.md","REVIEW.md","TESTS.md","MANIFEST.json","EVIDENCE.md")){Require-File "docsplan/nfabi/plans/evidence/$id/$f"}}
        if($rowState-in@("DELIVERED_FOR_REVIEW","APPROVED") -and (Test-Path "docsplan/nfabi/plans/evidence/$id/MANIFEST.json")){
            try{$deliveryManifest=Get-Content -Raw "docsplan/nfabi/plans/evidence/$id/MANIFEST.json"|ConvertFrom-Json
                ...
                $expectedArchive="docsplan/nfabi/plans/evidence/$id/attempts/$($deliveryManifest.attempt_id)"
                if($deliveryManifest.attempt_archive-ne$expectedArchive){$errors.Add("dossiê ${id}: attempt_archive incorreto")}
                if(-not(Test-Path -LiteralPath "docsplan/nfabi/plans/evidence/$id/evidence" -PathType Container)){$errors.Add("dossiê ${id}: diretório evidence ausente")}
                foreach($archiveFile in @("DELIVERY.md","REVIEW.md","TESTS.md","MANIFEST.json","EVIDENCE.md")){
                    $topFile="docsplan/nfabi/plans/evidence/$id/$archiveFile";$snapshotFile="$expectedArchive/$archiveFile";Require-File $snapshotFile
                ...
                foreach($deliveredAttempt in @($deliveredAttempts[$id])){
                    foreach($historicalFile in @("DELIVERY.md","REVIEW.md","TESTS.md","MANIFEST.json","EVIDENCE.md")){Require-File "docsplan/nfabi/plans/evidence/$id/attempts/$deliveredAttempt/$historicalFile"}
                }
    ...
    if($s001-in@("DELIVERED_FOR_REVIEW","APPROVED") -and (Test-Path "docsplan/nfabi/plans/DECISION-REGISTER.md")){
        $d=Get-Content -Raw "docsplan/nfabi/plans/DECISION-REGISTER.md"
    ...
    if($s001-in@("DELIVERED_FOR_REVIEW","APPROVED") -and (Test-Path "docsplan/nfabi/planning/QUESTION-LEDGER.md")){
        foreach($line in Get-Content "docsplan/nfabi/planning/QUESTION-LEDGER.md"){
    ...
    if($s001-in@("DELIVERED_FOR_REVIEW","APPROVED") -and (Test-Path "docsplan/nfabi/planning/RISK-REGISTER.md")){
        foreach($line in Get-Content "docsplan/nfabi/planning/RISK-REGISTER.md"){
    ...
    $markdown=@(Get-ChildItem -Recurse -File -Filter "*.md" -Path ".agents","docsplan/nfabi" -ErrorAction SilentlyContinue); if(Test-Path "AGENTS.md"){$markdown+=Get-Item "AGENTS.md"}
